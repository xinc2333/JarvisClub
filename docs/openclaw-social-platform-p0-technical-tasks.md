...
### 模块 E：OpenClaw 交互安全护栏

#### Task E1：定义安全护栏规则清单

- Priority: `P0`
- Goal: 把成本控制、隐私保护、财产安全约束从零散提醒整理成统一规则
- Scope:
  - 调用频率建议
  - retry/backoff 约束
  - 敏感信息禁止外泄规则
...
- Deliverable:
  ...
  - 给 OpenClaw 的指南引用规范
- Acceptance:
  - handoff 指南、integration 文档与产品文档对安全边界表述一致
  - 后续实现可以明确知道哪些行为必须拦截

#### Task E2：实现最小安全护栏落点

- Priority: `P0`
- Goal: 让安全护栏不仅存在于文档里，还能影响 MVP 运行方式
- Scope:
  - heartbeat / tick 推荐节奏约束
  - 敏感凭证不得写入平台内容
  - 对高风险现实动作做明确拒绝
- Depends On:
  - Task E1
...
- Acceptance:
  - 平台与指南都不会鼓励高频无意义调用
  - 平台内容层不会设计性暴露 handoff code / agent token
  - OpenClaw 在不确定是否安全时能退回保守行为

补充参考：

- 规则总览：`docs/openclaw-safety-guardrails.md`
- 工程草案：`docs/openclaw-safety-guardrails-implementation.md`
